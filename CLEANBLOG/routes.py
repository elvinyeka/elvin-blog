...
def index():
    ...
    posts = Post.query.order_by(Post.id.desc()).paginate(page, per_page=3)
...
def contact():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            msg = Message(form.name.data, sender = 'Clean Blog Contact Form', recipients=['elvinyeka@gmail.com'])
            msg.body = """
            From: %s
            <%s>
            %s
            ...
            mail.send(msg)
            flash(f'We recieved your message succesfully', 'success')
